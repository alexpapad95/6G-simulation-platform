...
substrate_srr.epsR = 2.2; %a high-frequency laminate Rogers RT/Duroid 5880
tand = 0.0009;
...
L1=17.15; %length of the outer ring
L2=16.92; %width of the outer ring
...
dx=5;% x-axis distance between elements
dz=5; % z-axis distance between elements
lumped_elements_width= 0.8;
...
for i=-((dim_meta-1)/2):((dim_meta-1)/2)
    ...
    for j=-((dim_meta-1)/2):((dim_meta-1)/2)
        ...
        if i~=(dim_meta-1)/2
            ...
            tunable_load_patches=tunable_load_patches+1;
            start = [L2/2+RIS1_x+i*(dx+L2)+dx/2-lumped_elements_width/2, RIS1_y-lumped_elements_width/2, L1/4+RIS1_z+z];
            stop = [ L2/2+RIS1_x+i*(dx+L2)+dx/2+lumped_elements_width/2, RIS1_y+lumped_elements_width/2,L1/4-width_patch+RIS1_z+z];
            %tunable_element_id=append ('tunable_elements_',int2str(tunable_load_patches)); %not compatible with Octave
            tunable_element_id=strcat ('tunable_elements_',int2str(tunable_load_patches)); %% Octave command
            CSX = AddLumpedElement( CSX, tunable_element_id, 1, 'C', lumped_c(1,tunable_load_patches), 'R', lumped_r(1,tunable_load_patches)+50);
            CSX = AddBox( CSX, tunable_element_id, 10, start, stop );
        ...
        if j~=(dim_meta-1)/2
            ...
            tunable_load_patches=tunable_load_patches+1;
            start = [L2/4+RIS1_x+x, RIS1_y-lumped_elements_width/2,L1/2+RIS1_z+j*(L1+dz)+dz/2-lumped_elements_width/2];
            stop = [L2/4+width_patch+RIS1_x+x, RIS1_y+lumped_elements_width/2,L1/2+RIS1_z+j*(L1+dz)+dz/2+lumped_elements_width/2];
            %tunable_element_id=append ('tunable_elements_',int2str(tunable_load_patches)); %not compatible with Octave
            tunable_element_id=strcat ('tunable_elements_',int2str(tunable_load_patches)); %% Octave command
            CSX = AddLumpedElement( CSX, tunable_element_id, 1, 'C', lumped_c(1,tunable_load_patches), 'R', lumped_r(1,tunable_load_patches)+50);
            CSX = AddBox( CSX, tunable_element_id, 10, start, stop );
...
for i=-((dim_meta-1)/2):((dim_meta-1)/2)
    ...
    for j=-((dim_meta-1)/2):((dim_meta-1)/2)
        ...
        if i~=(dim_meta-1)/2
            ...
            CSX = AddBox(CSX,'patch',4,start,stop);
            tunable_load_patches_2=tunable_load_patches_2+1;
            
            start = [L2/2+RIS2_x+i*(dx+L2)+dx/2-lumped_elements_width/2, RIS2_y+substrate_srr.thickness-lumped_elements_width/2, L1/4+RIS2_z+z];
            stop = [ L2/2+RIS2_x+i*(dx+L2)+dx/2+lumped_elements_width/2, RIS2_y+substrate_srr.thickness+lumped_elements_width/2,L1/4-width_patch+RIS2_z+z];
            %tunable_element_id=append ('tunable_elements_',int2str(tunable_load_patches+tunable_load_patches_2)); %not compatible with Octave
            tunable_element_id=strcat ('tunable_elements_',int2str(tunable_load_patches+tunable_load_patches_2)); %% Octave command
            CSX = AddLumpedElement( CSX, tunable_element_id, 1, 'C', lumped_c(2,tunable_load_patches_2), 'R', lumped_r(2,tunable_load_patches_2)+50);
            CSX = AddBox( CSX, tunable_element_id, 10, start, stop );
        ...
        if j~=(dim_meta-1)/2
            ...
            start = [L2/4+RIS2_x+x, RIS2_y+substrate_srr.thickness-lumped_elements_width/2,L1/2+RIS2_z+j*(L1+dz)+dz/2-lumped_elements_width/2];
            stop = [L2/4+width_patch+RIS2_x+x, RIS2_y+substrate_srr.thickness+lumped_elements_width/2,L1/2+RIS2_z+j*(L1+dz)+dz/2+lumped_elements_width/2];
            %tunable_element_id=append ('tunable_elements_',int2str(tunable_load_patches+tunable_load_patches_2)); %not compatible with Octave
            tunable_element_id=strcat ('tunable_elements_',int2str(tunable_load_patches+tunable_load_patches_2)); %% Octave command
            CSX = AddLumpedElement( CSX, tunable_element_id, 1, 'C', lumped_c(2,tunable_load_patches_2), 'R', lumped_r(2,tunable_load_patches_2)+50);
            CSX = AddBox( CSX, tunable_element_id, 10, start, stop );
