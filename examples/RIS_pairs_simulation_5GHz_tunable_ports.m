...
%% Tunability of the elements %%%%
lumped_c = randi([0 0], 2,dim_meta^2)/10;
lumped_c=lumped_c*1e-12;
lumped_r= randi([0 0], 2,dim_meta^2)/10;

width_lumped_element=0.5;
    ...
    for j=-((dim_meta-1)/2):((dim_meta-1)/2)
        ...
        for k=number_of_elements
            ...
            [CSX, port{k}] = AddLumpedPort(CSX, 1 ,k ,feed.R, start, stop, [0 1 0], true);
             %tunable elements for local configuration %%%
             %tunable_element_id=append ('tunable_elements_',int2str(k)); not compatible with Octave
             start = [-G1/2+RIS1_x+x, RIS1_y+width_lumped_element, -L1/2+RIS1_z+z];
             stop  = [G1/2+RIS1_x+x, +RIS1_y, -L1/2+width_outer+RIS1_z+z];
             tunable_element_id=strcat ('tunable_elements_',int2str(k)); %% Octave command
             CSX = AddLumpedElement( CSX, tunable_element_id, 1, 'C', lumped_c(1,k), 'R', lumped_r(1,k)+50);
...
for i=-((dim_meta-1)/2):((dim_meta-1)/2)
    ...
    for j=-((dim_meta-1)/2):((dim_meta-1)/2)
        ...
        for k=number_of_elements_2
            ...
            %%% tunable elements for local configuration %%%
            %tunable_element_id=append ('tunable_elements_',int2str(k+number_of_elements)); %not compatible with Octave
            start = [-G1/2+RIS2_x+x, RIS2_y+substrate_srr.thickness-width_lumped_element, -L1/2+RIS2_z+z];
            stop  = [G1/2+RIS2_x+x, RIS2_y+substrate_srr.thickness, -L1/2+width_outer+RIS2_z+z];
            tunable_element_id=strcat ('tunable_elements_',int2str(k+number_of_elements)); %% Octave command
            CSX = AddLumpedElement( CSX, tunable_element_id, 1, 'C', lumped_c(2,k), 'R', lumped_r(2,k)+50);
            CSX = AddBox( CSX, tunable_element_id, 10, start, stop );
